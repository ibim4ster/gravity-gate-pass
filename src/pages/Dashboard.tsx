...
import {
  ...
  ExternalLink, QrCode, Upload, Store, Settings, Percent
} from 'lucide-react';
...
const Dashboard = () => {
  ...
  const [tab, setTab] = useState<'overview' | 'events' | 'attendees' | 'users' | 'logs' | 'contacts' | 'settings'>('overview');
  ...
  const [eventForm, setEventForm] = useState({
    ...
    category: 'Bar/Restaurante', capacity: '500', status: 'cerrado',
    image_url: '', maps_url: '', lineup: '', min_age: '0',
    gallery_urls: '', offer_text: '', offer_active: false,
    tiers: [{ id: '', name: 'Pincho individual', price: '3', maxQuantity: '200', expiresAt: '', description: '', imageUrls: '' as string }],
  });
  const imageInputRef = useRef<HTMLInputElement>(null);
  const tierImageInputRef = useRef<HTMLInputElement>(null);
  const [uploadingTierIdx, setUploadingTierIdx] = useState<number | null>(null);
  ...
  const fetchData = async () => {
    if (isAdmin) {
      const [eventsRes, logsRes, ticketsRes, profilesRes, rolesRes, assignRes, contactRes, settingsRes] = await Promise.all([
      ...
      if (assignData) {
        ...
        } else { setEvents([]); setTickets([]); setScanLogs([]); }
      }
    }
    setLoading(false);
  };
  ...
  const filteredTicketsForOverview = selectedEventFilter === 'all' ? tickets : tickets.filter(t => t.event_id === selectedEventFilter);
  ...
  const resetEventForm = () => {
    setEventForm({
      ...
      offer_text: '', offer_active: false,
      tiers: [{ id: '', name: 'Pincho individual', price: '3', maxQuantity: '200', expiresAt: '', description: '', imageUrls: '' }],
    });
  ...
  const startEditEvent = (ev: EventWithTiers) => {
    ...
    setEventForm({
      ...
      venue: ev.venue, city: ev.city, category: ev.category, capacity: String(ev.capacity),
      status: ev.status, image_url: ev.image_url || '', maps_url: (ev as any).maps_url || '',
      lineup: ev.lineup || '', min_age: String(ev.min_age || 0),
      gallery_urls: (ev.gallery_urls || []).join('\n'),
      offer_text: (ev as any).offer_text || '', offer_active: !!(ev as any).offer_active,
      tiers: ev.price_tiers.map(t => ({
        id: t.id, name: t.name, price: String(t.price), maxQuantity: String(t.max_quantity),
        expiresAt: t.expires_at ? t.expires_at.slice(0, 16) : '', description: (t as any).description || '',
        imageUrls: ((t as any).image_urls || []).join('\n'),
      })),
    });
    setTab('events');
  ...
  const saveEvent = async () => {
    try {
      ...
      const eventData: any = {
        ...
        date: eventForm.date, time: eventForm.time, venue: eventForm.venue, city: eventForm.city,
        category: eventForm.category, capacity: parseInt(eventForm.capacity),
        status: eventForm.status, image_url: eventForm.image_url || null,
        maps_url: eventForm.maps_url || null, lineup: eventForm.lineup || null,
        min_age: parseInt(eventForm.min_age) || 0,
        gallery_urls: galleryArray.length > 0 ? galleryArray : null,
        offer_text: eventForm.offer_text || null, offer_active: eventForm.offer_active,
      };
      ...
      for (const tier of eventForm.tiers) {
        const tierImageUrls = tier.imageUrls.split('\n').map((u: string) => u.trim()).filter(Boolean);
        const tierData = {
          ...
          description: tier.description || null, image_urls: tierImageUrls.length > 0 ? tierImageUrls : [],
        } as any;
        if (tier.id) {
  ...
  const deleteEvent = async (id: string) => {
    ...
    toast.success('Bar eliminado'); fetchData();
  };
  ...
  const removeRole = async (userId: string, role: string) => {
    ...
    toast.success('Rol eliminado'); fetchData();
  };
  ...
  const deleteUser = async (userId: string) => {
    ...
    try {
      ...
      toast.success('Usuario eliminado'); fetchData();
  ...
  const resetUserPassword = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('admin-manage-user', { body: { action: 'reset-password', userId: resetPasswordUserId, newPassword: newPasswordValue } });
      if (error) throw error;
      if (data?.error) throw new Error(data.error);
      toast.success('Contraseña restablecida');
      setResetPasswordUserId(null); setNewPasswordValue('');
  ...
  const deleteContactRequest = async (id: string) => {
    ...
    toast.success('Solicitud eliminada'); fetchData();
  };
  ...
  const handleTierImageUpload = async (file: File, tierIdx: number) => {
    const ext = file.name.split('.').pop();
    const path = `packs/${Date.now()}-${Math.random().toString(36).slice(2)}.${ext}`;
    ...
    const tiers = [...eventForm.tiers];
    const current = tiers[tierIdx].imageUrls;
    tiers[tierIdx] = { ...tiers[tierIdx], imageUrls: current ? current + '\n' + publicUrl : publicUrl };
    setEventForm({ ...eventForm, tiers });
  ...
  const adminTabs = [
    ...
    { key: 'contacts' as const, label: 'Solicitudes', icon: Store },
        ...
        <div className="container py-6">
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="font-display text-2xl font-bold tracking-tight">{isAdmin ? 'Panel de Administración' : 'Panel de Staff'}</h1>
                <p className="text-muted-foreground text-sm">
                  {profile?.display_name || user.email}
                  <span className="ml-2 px-2 py-0.5 rounded-md bg-primary/10 text-primary text-[10px] font-bold uppercase">{isAdmin ? 'ADMIN' : 'STAFF'}</span>
                </p>
              </div>
            </div>
          </motion.div>
      ...
      <div className="container py-8 space-y-6">
          ...
          <div className="space-y-6">
            ...
            {showCreateEvent && (
              <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="bg-background rounded-2xl border border-border p-6 space-y-5">
                ...
                <div className="p-4 rounded-xl border border-border bg-muted/50 space-y-3">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <Label className="font-semibold">Oferta activa</Label>
                    </div>
                    <Switch checked={eventForm.offer_active} onCheckedChange={v => setEventForm({ ...eventForm, offer_active: v })} />
                  </div>
                  {eventForm.offer_active && (
                    <Input value={eventForm.offer_text} onChange={e => setEventForm({ ...eventForm, offer_text: e.target.value })} placeholder="Ej: 2x1 en cañas los jueves" />
                  )}
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Textarea value={eventForm.description} onChange={e => setEventForm({ ...eventForm, description: e.target.value })} className="min-h-[80px]" placeholder="Especialidades, ambiente..." />
                </div>
                <div className="space-y-2">
                ...
                <div className="space-y-3">
                  ...
                  {eventForm.tiers.map((tier, i) => (
                    <div key={i} className="p-3 rounded-xl bg-muted space-y-2">
                      ...
                      <div className="space-y-1">
                        <Label className="text-xs flex items-center gap-1"><Image className="w-3 h-3" /> Imágenes del pack</Label>
                        <div className="flex gap-2 items-center">
                          <input ref={i === uploadingTierIdx ? tierImageInputRef : undefined} type="file" accept="image/*" multiple className="hidden"
                            onChange={async (e) => {
                              const files = e.target.files;
                              if (!files) return;
                              for (const file of Array.from(files)) {
                                await handleTierImageUpload(file, i);
                              }
                            }} />
                          <Button type="button" variant="outline" size="sm" className="rounded-xl gap-1 text-xs" onClick={() => {
                            setUploadingTierIdx(i);
                            setTimeout(() => tierImageInputRef.current?.click(), 50);
                          }}>
                            <Upload className="w-3 h-3" /> Subir imágenes
                          </Button>
                        </div>
                        {tier.imageUrls && tier.imageUrls.trim() && (
                          <div className="flex gap-2 flex-wrap mt-1">
                            {tier.imageUrls.split('\n').filter(Boolean).map((url, idx) => (
                              <div key={idx} className="w-16 h-16 rounded-lg overflow-hidden border border-border relative group">
                                <img src={url} alt="" className="w-full h-full object-cover" />
                                <button onClick={() => {
                                  const urls = tier.imageUrls.split('\n').filter((_, j) => j !== idx).join('\n');
                                  const t = [...eventForm.tiers]; t[i] = { ...t[i], imageUrls: urls }; setEventForm({ ...eventForm, tiers: t });
                  ...
                  <Button variant="outline" size="sm" className="rounded-xl" onClick={() => setEventForm({ ...eventForm, tiers: [...eventForm.tiers, { id: '', name: '', price: '0', maxQuantity: '50', expiresAt: '', description: '', imageUrls: '' }] })}>
                    + Añadir pack
                  </Button>
                </div>
            ...
            <div className="space-y-3">
              {events.filter(e => !barSearch || e.title.toLowerCase().includes(barSearch.toLowerCase()) || e.venue.toLowerCase().includes(barSearch.toLowerCase())).map(event => {
                ...
                const hasOffer = !!(event as any).offer_active;
                  ...
                  <div key={event.id} className="bg-background rounded-2xl border border-border overflow-hidden hover:shadow-sm transition-shadow">
                        ...
                        <div className="flex gap-3 flex-1 min-w-0">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              {hasOffer && (
                                <span className="px-2 py-0.5 rounded-md bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400 text-[10px] font-bold uppercase flex items-center gap-1">
                                  <Percent className="w-3 h-3" /> Oferta
                                </span>
                              )}
                            </div>
                            <p className="text-xs text-muted-foreground">{event.venue}, {event.city} · {sold} vendidos · {revenue}€</p>
                          </div>
                        </div>
                    ...
                    {isExpanded && (
                      <div className="px-4 pb-4 border-t border-border pt-3 space-y-3">
                        {(event as any).offer_active && (event as any).offer_text && (
                          <div className="p-3 rounded-xl bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800">
                            <p className="text-sm font-medium text-amber-700 dark:text-amber-400 flex items-center gap-2"><Percent className="w-4 h-4" /> {(event as any).offer_text}</p>
                          </div>
                        )}
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                          {event.price_tiers.map(t => (
                            <div key={t.id} className="p-3 rounded-xl bg-muted space-y-1">
                              ...
                              <p className="text-xs text-muted-foreground">{t.sold}/{t.max_quantity} vendidos</p>
                            </div>
                          ))}
                        </div>
                        {assignedUsers.length > 0 && (
                          <div className="flex items-center gap-2 text-xs text-muted-foreground">
                            <Users className="w-3.5 h-3.5" /> Staff: {assignedUsers.map((u: any) => u.display_name || u.email).join(', ')}
                          </div>
                        )}
                      </div>
                    )}
              ...
              {filteredTickets.map(t => {
                ...
                return (
                  <div key={t.id} className="bg-background rounded-2xl border border-border overflow-hidden hover:shadow-sm transition-shadow">
                    <button onClick={() => setExpandedTicket(isOpen ? null : t.id)} className="w-full p-4 flex items-center justify-between text-left">
                    ...
                    {isOpen && (
                      <div className="px-4 pb-4 border-t border-border pt-3">
                        ...
                        {/* QR Code visible for admins */}
                        {isAdmin && (
                          <div className="mt-4 p-4 rounded-xl bg-muted flex flex-col items-center gap-3">
                            <p className="text-xs text-muted-foreground font-medium flex items-center gap-1"><QrCode className="w-3.5 h-3.5" /> Código QR del ticket</p>
                            <QRCodeSVG value={`${t.qr_code}|${t.qr_signature}`} size={180} className="rounded-lg" />
                            <p className="font-mono text-xs break-all select-all text-center">{t.qr_code}</p>
                          </div>
                        )}
                        {!isAdmin && (
                          <div className="mt-4 p-3 rounded-xl bg-muted space-y-1">
                            <div className="flex items-center gap-2 text-xs text-muted-foreground"><QrCode className="w-3.5 h-3.5" /> Código QR</div>
        ...
        {tab === 'users' && isAdmin && (
          <div className="space-y-6">
                  ...
                  <div key={u.user_id} className="bg-background rounded-2xl border border-border p-5 space-y-4 hover:shadow-sm transition-shadow">
                    <div className="flex items-start justify-between">
                      ...
                      <div className="flex items-center gap-1">
                        <Button size="sm" variant="ghost" className="h-8 w-8 p-0" onClick={() => { setEditingUser(u); setEditUserForm({ display_name: u.display_name || '', email: u.email || '' }); }}><Edit2 className="w-3.5 h-3.5" /></Button>
                        <Button size="sm" variant="ghost" className="h-8 w-8 p-0" onClick={() => { setResetPasswordUserId(u.user_id); setNewPasswordValue(''); }} title="Resetear contraseña"><Shield className="w-3.5 h-3.5" /></Button>
                        <Button size="sm" variant="ghost" className="h-8 w-8 p-0 text-destructive" onClick={() => deleteUser(u.user_id)} title="Eliminar usuario"><Trash2 className="w-3.5 h-3.5" /></Button>
                      </div>
                    </div>
                    <div className="space-y-2">
              ...
              <Dialog open={!!resetPasswordUserId} onOpenChange={() => setResetPasswordUserId(null)}>
                <DialogContent className="max-w-xs">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2"><Label>Nueva contraseña</Label><Input type="password" value={newPasswordValue} onChange={e => setNewPasswordValue(e.target.value)} placeholder="Mínimo 6 caracteres" /></div>
                    <div className="flex gap-2">
            ...
            {/* Bar assignments */}
            <div className="bg-background rounded-2xl border border-border p-6 space-y-4">
              <h3 className="font-display font-semibold flex items-center gap-2"><Link2 className="w-5 h-5 text-primary" /> Asignación de bares</h3>
              <p className="text-sm text-muted-foreground">Asigna staff a bares para limitar su visibilidad.</p>
              <div className="flex flex-wrap gap-3 items-end">
                <div className="space-y-1">
                  ...
                  <Select value={assignUserId} onValueChange={setAssignUserId}>
                    ...
                    <SelectContent>{staffOrOrgUsers.map(u => <SelectItem key={u.user_id} value={u.user_id}>{u.display_name || u.email}</SelectItem>)}</SelectContent>
                  </Select>
                </div>
                <div className="space-y-1">
                    ...
                    return (
                      <tr key={log.id} className="hover:bg-muted/30">
                        ...
                        <td className="p-3">
                          ...
                          }`}>{log.result === 'valid' ? 'Válido' : log.result === 'already_used' ? 'Ya usado' : 'Inválido'}</span>
                        </td>
                        <td className="p-3 text-muted-foreground">{staff?.display_name || staff?.email || log.staff_id.slice(0, 8)}</td>
                      </tr>
                    );
                  })}
                </tbody>
              </table>
              {scanLogs.length === 0 && <p className="text-center text-muted-foreground py-8">Sin registros de escaneo.</p>}
            </div>
          </div>
        )}

        {/* CONTACTS - Admin only */}
        {tab === 'contacts' && isAdmin && (
          <div className="space-y-4">
            <h3 className="font-display font-semibold text-lg flex items-center gap-2"><Store className="w-5 h-5 text-primary" /> Solicitudes de nuevos bares</h3>
            {contactRequests.length === 0 ? (
              <p className="text-center text-muted-foreground py-12">No hay solicitudes pendientes.</p>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {contactRequests.map((req: any) => (
                  <div key={req.id} className="bg-background rounded-2xl border border-border p-5 space-y-3">
                    <div className="flex items-start justify-between">
                      <div>
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-xs text-muted-foreground">{format(new Date(req.created_at), "d MMM yyyy", { locale: es })}</span>
                        <Button size="sm" variant="ghost" className="h-7 w-7 p-0 text-destructive" onClick={() => deleteContactRequest(req.id)}><Trash2 className="w-3 h-3" /></Button>
                      </div>
                    </div>
                    {req.bar_name && <p className="text-sm"><span className="text-muted-foreground">Bar:</span> <span className="font-medium">{req.bar_name}</span></p>}
                    {req.phone && <p className="text-sm text-muted-foreground">Tel: {req.phone}</p>}
                    <p className="text-sm bg-muted p-3 rounded-xl">{req.message}</p>
                  </div>
                ))}
              </div>
            )}
        ...
        {tab === 'settings' && isAdmin && (
          <div className="max-w-lg space-y-6">
            <h3 className="font-display font-semibold text-lg flex items-center gap-2"><Settings className="w-5 h-5 text-primary" /> Configuración del sitio</h3>
            <div className="bg-background rounded-2xl border border-border p-6 space-y-4">
              <div className="space-y-2">
                <Label>Nombre del sitio</Label>
                <Input value={siteSettings.site_name || ''} onChange={e => setSiteSettings({ ...siteSettings, site_name: e.target.value })} />
              </div>
              <div className="space-y-2">
                <Label>URL del logo</Label>
                <Input value={siteSettings.logo_url || ''} onChange={e => setSiteSettings({ ...siteSettings, logo_url: e.target.value })} />
                {siteSettings.logo_url && (
                  <div className="h-16 flex items-center">
                    <img src={siteSettings.logo_url} alt="Logo" className="h-full object-contain" />
                  </div>
                )}
              </div>
              <div className="space-y-2">
              ...
              <Button onClick={saveSiteSettings} disabled={settingsLoading} className="rounded-xl gap-2">
                <Save className="w-4 h-4" /> {settingsLoading ? 'Guardando...' : 'Guardar configuración'}
              </Button>
            </div>
          </div>
        )}
